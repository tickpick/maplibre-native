...
int processIsTranslated() {
  ...
  if (sysctlbyname("sysctl.proc_translated", &ret, &size, NULL, 0) != -1) return ret;
...
const MLNMapViewDecelerationRate MLNMapViewDecelerationRateNormal =
...
const MLNExceptionName MLNMissingLocationServicesUsageDescriptionException =
    @"MLNMissingLocationServicesUsageDescriptionException";
const MLNExceptionName MLNUserLocationAnnotationTypeException =
    @"MLNUserLocationAnnotationTypeException";
const MLNExceptionName MLNUnderlyingMapUnavailableException =
    @"MLNUnderlyingMapUnavailableException";
...
@property (nonatomic)
    NSMutableDictionary<NSString *, MLNAnnotationImage *> *annotationImagesByIdentifier;
...
@implementation MLNMapView {
  ...
  /// True if a willChange notification has been issued for shape annotation layers and a didChange
...
- (nonnull NSURL *)styleURL {
  ...
  if (self.mbglMap.getStyle().getJSON().length() > 0 && self.mbglMap.getStyle().getURL().empty()) {
...
- (void)commonInitWithOptions:(MLNMapOptions *)mlnMapoptions {
  ...
  dispatch_once(&onceToken, ^{
    NSLog(@"%@ WARNING UIViewController.automaticallyAdjustsScrollViewInsets is deprecated use "
          @"MLNMapView.automaticallyAdjustContentInset instead.",
  ...
  _attributionButton.accessibilityLabel = NSLocalizedStringWithDefaultValue(
      @"INFO_A11Y_LABEL", nil, nil, @"About this map", @"Accessibility label");
  _attributionButton.accessibilityHint = NSLocalizedStringWithDefaultValue(
      @"INFO_A11Y_HINT", nil, nil, @"Shows credits, a feedback form, and more",
      @"Accessibility hint");
  [_attributionButton addTarget:self
  ...
  UILongPressGestureRecognizer *attributionLongPress =
      [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(showAttribution:)];
...
#if TARGET_IPHONE_SIMULATOR & (TARGET_CPU_X86 | TARGET_CPU_X86_64)
  if (isM1Simulator) {
    _rotate = [[MBGLRotationGesture alloc] initWithTarget:self
                                                   action:@selector(handleRotateGesture:)];
  } else {
    _rotate = [[UIRotationGestureRecognizer alloc] initWithTarget:self
                                                           action:@selector(handleRotateGesture:)];
  }
#else
  _rotate = [[UIRotationGestureRecognizer alloc] initWithTarget:self
  ...
  _twoFingerDrag =
      [[UIPanGestureRecognizer alloc] initWithTarget:self
                                              action:@selector(handleTwoFingerDragGesture:)];
  ...
  _twoFingerTap =
      [[UITapGestureRecognizer alloc] initWithTarget:self
                                              action:@selector(handleTwoFingerTapGesture:)];
  ...
  _quickZoom =
      [[UILongPressGestureRecognizer alloc] initWithTarget:self
                                                    action:@selector(handleQuickZoomGesture:)];
  ...
  // As of 3.7.5, we intentionally do not listen for `UIApplicationWillResignActiveNotification` or
  // call `pauseRendering:` in response to it, as doing so causes a loop when asking for location
  // permission. See: https://github.com/mapbox/mapbox-gl-native/issues/11225
...
- (void)setDelegate:(nullable id<MLNMapViewDelegate>)delegate {
  ...
  _delegateHasAlphasForShapeAnnotations =
      [_delegate respondsToSelector:@selector(mapView:alphaForShapeAnnotation:)];
  _delegateHasStrokeColorsForShapeAnnotations =
      [_delegate respondsToSelector:@selector(mapView:strokeColorForShapeAnnotation:)];
  _delegateHasFillColorsForShapeAnnotations =
      [_delegate respondsToSelector:@selector(mapView:fillColorForPolygonAnnotation:)];
  _delegateHasLineWidthsForShapeAnnotations =
      [_delegate respondsToSelector:@selector(mapView:lineWidthForPolylineAnnotation:)];
}
...
- (void)setCompassViewMargins:(CGPoint)compassViewMargins {
  MLNLogDebug(@"Setting compassViewOffset: (x:%f, y:%f)", compassViewMargins.x,
              compassViewMargins.y);
...
- (void)setAttributionButtonMargins:(CGPoint)attributionButtonMargins {
  MLNLogDebug(@"Setting attributionButtonMargins: (x:%f, y:%f)", attributionButtonMargins.x,
              attributionButtonMargins.y);
...
#pragma clang diagnostic pop
  ...
  if (!automaticallyAdjustContentInset) {
    ...
    inset = UIEdgeInsetsMake(fmaxf(inset.top, 0), fmaxf(inset.left, 0), fmaxf(inset.bottom, 0),
                             fmaxf(inset.right, 0));
  }

  switch (position) {
    case MLNOrnamentPositionTopLeft:
      [updatedConstraints addObject:[view.topAnchor constraintEqualToAnchor:self.mgl_safeTopAnchor
                                                                   constant:margins.y + inset.top]];
      [updatedConstraints
          addObject:[view.leadingAnchor constraintEqualToAnchor:self.mgl_safeLeadingAnchor
                                                       constant:margins.x + inset.left]];
      break;
    case MLNOrnamentPositionTopRight:
      [updatedConstraints addObject:[view.topAnchor constraintEqualToAnchor:self.mgl_safeTopAnchor
                                                                   constant:margins.y + inset.top]];
      [updatedConstraints
          addObject:[self.mgl_safeTrailingAnchor constraintEqualToAnchor:view.trailingAnchor
                                                                constant:margins.x + inset.right]];
      break;
    case MLNOrnamentPositionBottomLeft:
      [updatedConstraints
          addObject:[self.mgl_safeBottomAnchor constraintEqualToAnchor:view.bottomAnchor
                                                              constant:margins.y + inset.bottom]];
      [updatedConstraints
          addObject:[view.leadingAnchor constraintEqualToAnchor:self.mgl_safeLeadingAnchor
                                                       constant:margins.x + inset.left]];
      break;
  ...
  [self updateConstraintsForOrnament:self.compassView
                         constraints:self.compassViewConstraints
                            position:self.compassViewPosition
                                size:[self sizeForOrnament:self.compassView
                                               constraints:self.compassViewConstraints]
  ...
  [self updateConstraintsForOrnament:self.logoView
                         constraints:self.logoViewConstraints
                            position:self.logoViewPosition
                                size:[self sizeForOrnament:self.logoView
                                               constraints:self.logoViewConstraints]
  ...
  [self updateConstraintsForOrnament:self.attributionButton
                         constraints:self.attributionButtonConstraints
                            position:self.attributionButtonPosition
                                size:[self sizeForOrnament:self.attributionButton
                                               constraints:self.attributionButtonConstraints]
                             margins:self.attributionButtonMargins];
}

- (CGSize)sizeForOrnament:(UIView *)view constraints:(NSMutableArray *)constraints {
...
- (void)layoutSubviews {
  ...
  MLNAssert(
      self.attributionButton.isHidden ||
          CGRectContainsRect(self.bounds, self.attributionButton.mgl_frameForIdentifyTransform),
      @"The attribution is not in the visible area of the mapview. Please check your position and "
      @"offset settings");
  MLNAssert(self.scaleBar.isHidden ||
                CGRectContainsRect(self.bounds, self.scaleBar.mgl_frameForIdentifyTransform),
            @"The scaleBar is not in the visible area of the mapview. Please check your position "
            @"and offset settings");
  MLNAssert(self.compassView.isHidden ||
                self.compassView.compassVisibility == MLNOrnamentVisibilityHidden ||
                CGRectContainsRect(self.bounds, self.compassView.mgl_frameForIdentifyTransform),
            @"The compassView is not in the visible area of the mapview. Please check your "
            @"position and offset settings");
  MLNAssert(self.logoView.isHidden ||
                CGRectContainsRect(self.bounds, self.logoView.mgl_frameForIdentifyTransform),
            @"The logoView is not in the visible area of the mapview. Please check your position "
            @"and offset settings");
}
...
- (void)setAutomaticallyAdjustsContentInset:(BOOL)automaticallyAdjustsContentInset {
  MLNLogDebug(@"Setting automaticallyAdjustsContentInset: %@",
              MLNStringFromBOOL(automaticallyAdjustsContentInset));
  _automaticallyAdjustContentInsetHolder =
      [NSNumber numberWithBool:automaticallyAdjustsContentInset];
}
...
- (void)setContentInset:(UIEdgeInsets)contentInset
  ...
  MLNLogDebug(@"Setting contentInset: %@ animated:", NSStringFromUIEdgeInsets(contentInset),
...
- (void)updateDisplayLinkPreferredFramesPerSecond {
  ...
  if (_preferredFramesPerSecond == MLNMapViewPreferredFramesPerSecondDefault) {
    ...
    newFrameRate = UIDevice.currentDevice.mgl_isLegacyDevice
...
- (void)updatePresentsWithTransaction {
  BOOL hasEnoughViewAnnotations = (self.annotationContainerView.annotationViews.count >
                                   MLNPresentsWithTransactionAnnotationCount);
...
- (void)deviceOrientationDidChange:(__unused NSNotification *)notification {
  ...
  UIInterfaceOrientationMask validOrientation =
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan {
  ...
  if (pan.state == UIGestureRecognizerStateBegan) {
    ...
    if (drift) {
      CGPoint offset = CGPointMake(velocity.x * self.decelerationRate / 4,
                                   velocity.y * self.decelerationRate / 4);
      ...
      if ([self _shouldChangeFromCamera:oldCamera toCamera:toCamera]) {
        switch (self.panScrollingMode) {
          ...
          case MLNPanScrollingModeHorizontal:
            ...
            break;
          default:
            self.mbglMap.moveBy({offset.x, offset.y},
                                MLNDurationFromTimeInterval(self.decelerationRate));
        }
      }
    }
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)pinch {
  ...
  if (pinch.state == UIGestureRecognizerStateBegan) {
    ...
    if ([self _shouldChangeFromCamera:oldCamera toCamera:toCamera]) {
      ...
      // meaningless.
      if (self.userTrackingMode == MLNUserTrackingModeNone &&
          pinch.numberOfTouches == _previousPinchNumberOfTouches) {
        self.mbglMap.moveBy({centerPoint.x - _previousPinchCenterPoint.x,
                             centerPoint.y - _previousPinchCenterPoint.y});
      }
    }
    [self cameraIsChanging];
...
- (void)handleRotateGesture:(UIRotationGestureRecognizer *)rotate {
  ...
  if ([[NSUserDefaults standardUserDefaults] objectForKey:MLNRotationThresholdWhileZoomingKey]) {
    self.rotationThresholdWhileZooming = [[[NSUserDefaults standardUserDefaults]
        objectForKey:MLNRotationThresholdWhileZoomingKey] floatValue];
  }
  ...
  if (rotate.state == UIGestureRecognizerStateChanged) {
    ...
    if (decelerationRate != MLNMapViewDecelerationRateImmediate && fabs(velocity) > 3) {
      ...
      if ([self _shouldChangeFromCamera:oldCamera toCamera:toCamera]) {
        ...
        [self animateWithDelay:decelerationRate
                    animations:^{
...
- (void)handleSingleTapGesture:(UITapGestureRecognizer *)singleTap {
  ...
  if (self.mapViewProxyAccessibilityElement.accessibilityElementIsFocused) {
    ...
    if (_userLocationAnnotationIsSelected) {
      ...
      if (_selectedAnnotationTag != MLNAnnotationTagNotFound) {
        nextElement =
            _annotationContextsByAnnotationTag.at(_selectedAnnotationTag).accessibilityElement;
      }
    }
  ...
  id<MLNAnnotation> annotation = [self annotationForGestureRecognizer:singleTap
                                                    persistingResults:YES];
  if (annotation) {
    CGPoint calloutPoint = [singleTap locationInView:self];
    CGRect positionRect = [self positioningRectForAnnotation:annotation
                                         defaultCalloutPoint:calloutPoint];
...
/**
 ...
 @param persist True to remember the cycleable set of annotations. @see
 annotationTagAtPoint:persistingResults
 */
- (nullable id<MLNAnnotation>)annotationForGestureRecognizer:(UITapGestureRecognizer *)singleTap
                                           persistingResults:(BOOL)persist {
  CGPoint tapPoint = [singleTap locationInView:self];

  if (self.userLocationVisible) {
    CGPoint tapPointForUserLocation;
    if (self.userLocationAnnotationView.hitTestLayer ==
    ...
    CALayer *hitLayer =
        [self.userLocationAnnotationView.hitTestLayer hitTest:tapPointForUserLocation];
  ...
  MLNAnnotationTag hitAnnotationTag = [self annotationTagAtPoint:tapPoint
                                               persistingResults:persist];
  if (hitAnnotationTag != MLNAnnotationTagNotFound) {
    if (hitAnnotationTag != _selectedAnnotationTag) {
      ...
      MLNAssert(annotation, @"Cannot select nonexistent annotation with tag %llu",
                hitAnnotationTag);
      return annotation;
    }
  }
...
- (void)handleDoubleTapGesture:(UITapGestureRecognizer *)doubleTap {
  ...
  if ([self _shouldChangeFromCamera:oldCamera toCamera:toCamera]) {
    ...
    [self animateWithDelay:MLNAnimationDuration
                animations:^{
                  [weakSelf unrotateIfNeededForGesture];
                }];
  } else {
...
- (void)handleQuickZoomGesture:(UILongPressGestureRecognizer *)quickZoom {
  ...
  if (quickZoom.state == UIGestureRecognizerStateBegan) {
    ...
    [self cameraIsChanging];
  } else if (quickZoom.state == UIGestureRecognizerStateEnded ||
             quickZoom.state == UIGestureRecognizerStateCancelled) {
    [self notifyGestureDidEndWithDrift:NO];
    [self unrotateIfNeededForGesture];
  }
}
...
- (void)handleTwoFingerDragGesture:(UIPanGestureRecognizer *)twoFingerDrag {
  ...
  if (twoFingerDrag.state == UIGestureRecognizerStateBegan ||
    ...
    CLLocationDegrees fingerSlopeAngle = [self angleBetweenPoints:leftTouchPoint
                                                         endPoint:rightTouchPoint];
...
- (MLNMapCamera *)cameraByPanningWithTranslation:(CGPoint)endPoint
  ...
  CLLocationCoordinate2D panCoordinate = [self convertPoint:endCameraPoint
                                       toCoordinateFromView:pan.view];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
  if (gestureRecognizer == _twoFingerDrag) {
    ...
    if (!self.selectedAnnotation) {
      id<MLNAnnotation> annotation =
          [self annotationForGestureRecognizer:(UITapGestureRecognizer *)gestureRecognizer
                             persistingResults:NO];
      if (!annotation) {
        return NO;
      }
    }
...
- (void)showAttribution:(id)sender {
  ...
  for (MLNAttributionInfo *attributionInfo in attributionInfos) {
    UIAlertAction *action =
        [UIAlertAction actionWithTitle:[attributionInfo.title.string
                                           mgl_titleCasedStringWithLocale:[NSLocale currentLocale]]
  ...
  NSString *cancelTitle = NSLocalizedStringWithDefaultValue(
      @"CANCEL", nil, nil, @"Cancel", @"Title of button for dismissing attribution action sheet");
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"coordinate"] &&
    ...
    if (annotation == [self annotationWithTag:annotationTag]) {
      ...
      if (annotationTag != MLNAnnotationTagNotFound) {
        MLNAnnotationContext &annotationContext =
            _annotationContextsByAnnotationTag.at(annotationTag);
        if (annotationContext.annotationView) {
          // Redundantly move the associated annotation view outside the scope of the animation-less
          // transaction block in -updateAnnotationViews.
          annotationContext.annotationView.center =
              [self convertCoordinate:annotationContext.annotation.coordinate toPointToView:self];
        }

        MLNAnnotationImage *annotationImage = [self imageOfAnnotationWithTag:annotationTag];
        NSString *symbolName = annotationImage.styleIconIdentifier;

        // Update the annotation’s backing geometry to match the annotation model object. Any
        // associated annotation view is also moved by side effect. However, -updateAnnotationViews
    ...
    if ([keyPath isEqualToString:@"screen"] || [keyPath isEqualToString:@"windowScene"]) {
      [self destroyDisplayLink];
...
- (NSString *)accessibilityValue {
  ...
  [facts addObject:[NSString stringWithFormat:NSLocalizedStringWithDefaultValue(
                                                  @"MAP_A11Y_VALUE_ZOOM", nil, nil, @"Zoom %dx.",
                                                  @"Map accessibility value; {zoom level}"),
                                              (int)zoomLevel]];

  NSInteger annotationCount = self.accessibilityAnnotationCount;
  if (annotationCount) {
    [facts addObject:[NSString stringWithFormat:
                                   NSLocalizedStringWithDefaultValue(
                                       @"MAP_A11Y_VALUE_ANNOTATIONS", nil, nil,
                                       @"%ld annotation(s) visible.",
                                       @"Map accessibility value; {number of visible annotations}"),
                                   (long)self.accessibilityAnnotationCount]];
  }
  ...
  if (placeFeatures.count) {
    ...
    NSString *placesString = [placesArray
        componentsJoinedByString:NSLocalizedStringWithDefaultValue(@"LIST_SEPARATOR", nil, nil,
                                                                   @", ", @"List separator")];
    [facts addObject:[NSString
                         stringWithFormat:NSLocalizedStringWithDefaultValue(
                                              @"MAP_A11Y_VALUE_PLACES", nil, nil,
                                              @"Places visible: %@.",
  ...
  if (!_visiblePlaceFeatures) {
    ...
    _visiblePlaceFeatures =
        [self visibleFeaturesInRect:self.bounds
            inStyleLayersWithIdentifiers:[NSSet setWithArray:placeStyleLayerIdentifiers]];
  }
...
- (NSArray<id<MLNFeature>> *)visibleRoadFeatures {
  if (!_visibleRoadFeatures) {
    ...
    _visibleRoadFeatures =
        [self visibleFeaturesInRect:self.bounds
            inStyleLayersWithIdentifiers:[NSSet setWithArray:roadStyleLayerIdentifiers]];
  }
...
- (UIBezierPath *)accessibilityPath {
  ...
  if (self.calloutViewForSelectedAnnotation) {
    UIBezierPath *calloutViewPath =
        [UIBezierPath bezierPathWithRect:self.calloutViewForSelectedAnnotation.frame];
...
- (NSInteger)accessibilityElementCount {
  ...
  return !!self.userLocationAnnotationView + self.accessibilityAnnotationCount +
         self.visiblePlaceFeatures.count + self.visibleRoadFeatures.count +
         2 /* compass, attributionButton */;
}
...
- (id)accessibilityElementAtIndex:(NSInteger)index {
  ...
  NSRange userLocationAnnotationRange =
      NSMakeRange(compassIndex + 1, !!self.userLocationAnnotationView);
  ...
  // Visible annotations
  std::vector<MLNAnnotationTag> visibleAnnotations = [self annotationTagsInRect:self.bounds];
  NSRange visibleAnnotationRange =
      NSMakeRange(NSMaxRange(userLocationAnnotationRange), visibleAnnotations.size());
  if (NSLocationInRange(index, visibleAnnotationRange)) {
    std::sort(visibleAnnotations.begin(), visibleAnnotations.end());
    std::sort(visibleAnnotations.begin(), visibleAnnotations.end(),
              [&](const MLNAnnotationTag tagA, const MLNAnnotationTag tagB) {
    ...
    MLNAssert(
        ...
        @"Can’t get accessibility element for nonexistent or invisible annotation at index %li.",
        (long)index);
  ...
  NSRange visiblePlaceFeatureRange =
      NSMakeRange(NSMaxRange(visibleAnnotationRange), visiblePlaceFeatures.count);
  if (NSLocationInRange(index, visiblePlaceFeatureRange)) {
    visiblePlaceFeatures = [visiblePlaceFeatures
        sortedArrayUsingComparator:^NSComparisonResult(id<MLNFeature> _Nonnull featureA,
                                                       id<MLNFeature> _Nonnull featureB) {
  ...
  NSRange visibleRoadFeatureRange =
      NSMakeRange(NSMaxRange(visiblePlaceFeatureRange), visibleRoadFeatures.count);
  if (NSLocationInRange(index, visibleRoadFeatureRange)) {
    visibleRoadFeatures = [visibleRoadFeatures
        sortedArrayUsingComparator:^NSComparisonResult(id<MLNFeature> _Nonnull featureA,
                                                       id<MLNFeature> _Nonnull featureB) {
          CGPoint pointA = [self convertCoordinate:featureA.coordinate toPointToView:self];
          CGPoint pointB = [self convertCoordinate:featureB.coordinate toPointToView:self];
  ...
  MLNAssert(NO,
            @"Index %ld not in recognized accessibility element ranges. "
...
- (id)accessibilityElementForAnnotationWithTag:(MLNAnnotationTag)annotationTag {
  MLNAssert(_annotationContextsByAnnotationTag.count(annotationTag),
            @"Missing annotation for tag %llu.", annotationTag);
  ...
  if (!annotationContext.accessibilityElement) {
    annotationContext.accessibilityElement =
        [[MLNAnnotationAccessibilityElement alloc] initWithAccessibilityContainer:self
                                                                              tag:annotationTag];
  }

  // Update the accessibility element.
  MLNAnnotationImage *annotationImage = [self imageOfAnnotationWithTag:annotationTag];
  CGRect annotationFrame = [self frameOfImage:annotationImage.image
                         centeredAtCoordinate:annotation.coordinate];
  CGPoint annotationFrameCenter =
      CGPointMake(CGRectGetMidX(annotationFrame), CGRectGetMidY(annotationFrame));
...
- (id)accessibilityElementForPlaceFeature:(id<MLNFeature>)feature {
  ...
  MLNFeatureAccessibilityElement *element =
      [_featureAccessibilityElements objectsPassingTest:^BOOL(
                                         MLNFeatureAccessibilityElement *_Nonnull testElement,
                                         BOOL *_Nonnull stop) {
        return testElement.feature.identifier && ![testElement.feature.identifier isEqual:@0] &&
               [testElement.feature.identifier isEqual:feature.identifier];
      }].anyObject;
  if (!element) {
    element = [[MLNPlaceFeatureAccessibilityElement alloc] initWithAccessibilityContainer:self
                                                                                  feature:feature];
  }
  CGPoint center = [self convertCoordinate:feature.coordinate toPointToView:self];
  CGRect annotationFrame =
      CGRectInset({center, CGSizeZero}, -MLNAnnotationAccessibilityElementMinimumSize.width / 2,
                  -MLNAnnotationAccessibilityElementMinimumSize.width / 2);
  CGRect screenRect = UIAccessibilityConvertFrameToScreenCoordinates(annotationFrame, self);
...
- (id)accessibilityElementForRoadFeature:(id<MLNFeature>)feature {
  ...
  if (!element) {
    element = [[MLNRoadFeatureAccessibilityElement alloc] initWithAccessibilityContainer:self
                                                                                 feature:feature];
  }
  ...
  if (path) {
    CGPathRef strokedCGPath = CGPathCreateCopyByStrokingPath(
        path.CGPath, NULL, MLNAnnotationAccessibilityElementMinimumSize.width, kCGLineCapButt,
        kCGLineJoinMiter, 0);
...
- (NSInteger)indexOfAccessibilityElement:(id)element {
  ...
  if (tag != MLNAnnotationTagNotFound) {
    ...
    return visibleAnnotationRange.location +
           std::distance(visibleAnnotations.begin(), foundElement);
  }
  ...
  if ([element isKindOfClass:[MLNPlaceFeatureAccessibilityElement class]]) {
    ...
    if (featureIndex == NSNotFound) {
      featureIndex = [visiblePlaceFeatures
          indexOfObjectPassingTest:^BOOL(id<MLNFeature> _Nonnull visibleFeature, NSUInteger idx,
                                         BOOL *_Nonnull stop) {
            return visibleFeature.identifier && ![visibleFeature.identifier isEqual:@0] &&
                   [visibleFeature.identifier isEqual:feature.identifier];
          }];
    }
    if (featureIndex == NSNotFound) {
      return NSNotFound;
  ...
  if ([element isKindOfClass:[MLNRoadFeatureAccessibilityElement class]]) {
    ...
    if (featureIndex == NSNotFound) {
      featureIndex = [visibleRoadFeatures
          indexOfObjectPassingTest:^BOOL(id<MLNFeature> _Nonnull visibleFeature, NSUInteger idx,
                                         BOOL *_Nonnull stop) {
            return visibleFeature.identifier && ![visibleFeature.identifier isEqual:@0] &&
                   [visibleFeature.identifier isEqual:feature.identifier];
...
- (MLNMapViewProxyAccessibilityElement *)mapViewProxyAccessibilityElement {
  if (!_mapViewProxyAccessibilityElement) {
    _mapViewProxyAccessibilityElement =
        [[MLNMapViewProxyAccessibilityElement alloc] initWithAccessibilityContainer:self];
  }
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)coordinate animated:(BOOL)animated {
  MLNLogDebug(@"Setting centerCoordinate: %@ animated: %@",
              MLNStringFromCLLocationCoordinate2D(coordinate), MLNStringFromBOOL(animated));
  [self setCenterCoordinate:coordinate zoomLevel:self.zoomLevel animated:animated];
}

- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate {
  MLNLogDebug(@"Setting centerCoordinate: %@",
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  MLNLogDebug(@"Setting centerCoordinate: %@ zoomLevel: %f animated: %@",
              MLNStringFromCLLocationCoordinate2D(centerCoordinate), zoomLevel,
              MLNStringFromBOOL(animated));
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  MLNLogDebug(@"Setting centerCoordinate: %@ zoomLevel: %f direction: %f animated: %@",
              MLNStringFromCLLocationCoordinate2D(centerCoordinate), zoomLevel, direction,
              MLNStringFromBOOL(animated));
  [self setCenterCoordinate:centerCoordinate
                  zoomLevel:zoomLevel
                  direction:direction
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  MLNLogDebug(@"Setting centerCoordinate: %@ zoomLevel: %f direction: %f animated: %@ "
              @"completionHandler: %@",
              MLNStringFromCLLocationCoordinate2D(centerCoordinate), zoomLevel, direction,
              MLNStringFromBOOL(animated), completion);
  self.userTrackingMode = MLNUserTrackingModeNone;
...
- (void)_setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  if ([self.camera isEqualToMapCamera:camera] &&
      UIEdgeInsetsEqualToEdgeInsets(_contentInset, insets)) {
...
- (void)setVisibleCoordinateBounds:(MLNCoordinateBounds)bounds animated:(BOOL)animated {
  [self setVisibleCoordinateBounds:bounds
...
- (void)setVisibleCoordinateBounds:(MLNCoordinateBounds)bounds
  ...
  MLNLogDebug(@"Setting visibleCoordinateBounds: %@ edgePadding: %@ animated: %@",
              MLNStringFromCoordinateBounds(bounds), NSStringFromUIEdgeInsets(insets),
              MLNStringFromBOOL(animated));
...
- (void)setVisibleCoordinates:(const CLLocationCoordinate2D *)coordinates
                  ...
                  edgePadding:(UIEdgeInsets)insets
                     animated:(BOOL)animated {
  MLNLogDebug(@"Setting: %lu coordinates edgePadding: %@ animated: %@", count,
              NSStringFromUIEdgeInsets(insets), MLNStringFromBOOL(animated));
  [self setVisibleCoordinates:coordinates
                        count:count
                  edgePadding:insets
                    direction:self.direction
...
- (void)setVisibleCoordinates:(const CLLocationCoordinate2D *)coordinates
                  ...
                  edgePadding:(UIEdgeInsets)insets
                    direction:(CLLocationDirection)direction
                     duration:(NSTimeInterval)duration
      animationTimingFunction:(nullable CAMediaTimingFunction *)function {
  MLNLogDebug(@"Setting: %lu coordinates edgePadding: %@ direction: %f duration: %f "
              @"animationTimingFunction: %@",
              count, NSStringFromUIEdgeInsets(insets), direction, duration, function);
  [self setVisibleCoordinates:coordinates
                        count:count
                  edgePadding:insets
                    direction:direction
...
- (void)setVisibleCoordinates:(const CLLocationCoordinate2D *)coordinates
      ...
      animationTimingFunction:(nullable CAMediaTimingFunction *)function
            completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Setting: %lu coordinates edgePadding: %@ direction: %f duration: %f "
              @"animationTimingFunction: %@ completionHandler: %@",
              count, NSStringFromUIEdgeInsets(insets), direction, duration, function, completion);
  self.userTrackingMode = MLNUserTrackingModeNone;

  self.cameraChangeReasonBitmask |= MLNCameraChangeReasonProgrammatic;

  [self _setVisibleCoordinates:coordinates
                         count:count
                   edgePadding:insets
                     direction:direction
                      duration:duration
...
- (void)_setVisibleCoordinates:(const CLLocationCoordinate2D *)coordinates
                         count:(NSUInteger)count
                   edgePadding:(UIEdgeInsets)insets
                     direction:(CLLocationDirection)direction
                      duration:(NSTimeInterval)duration
...
- (void)_setDirection:(CLLocationDirection)direction
...
+ (NSSet<NSString *> *)keyPathsForValuesAffectingCamera {
  return [NSSet setWithObjects:@"longitude", @"latitude", @"centerCoordinate", @"zoomLevel",
                               @"direction", nil];
}
...
- (void)setCamera:(MLNMapCamera *)camera animated:(BOOL)animated {
  ...
  [self setCamera:camera
                 withDuration:animated ? MLNAnimationDuration : 0
      animationTimingFunction:nil];
}
...
- (void)setCamera:(MLNMapCamera *)camera
    ...
    animationTimingFunction:(nullable CAMediaTimingFunction *)function {
  MLNLogDebug(@"Setting camera: %@ duration: %f animationTimingFunction: %@", camera, duration,
              function);
  [self setCamera:camera
...
- (void)setCamera:(MLNMapCamera *)camera
    ...
    animationTimingFunction:(nullable CAMediaTimingFunction *)function
          completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Setting camera: %@ duration: %f animationTimingFunction: %@ completionHandler: %@",
              camera, duration, function, completion);
  [self setCamera:camera
                 withDuration:duration
      animationTimingFunction:function
                  edgePadding:UIEdgeInsetsZero
...
- (void)setCamera:(MLNMapCamera *)camera
  ...
  MLNLogDebug(@"Setting camera: %@ duration: %f animationTimingFunction: %@ edgePadding: %@ "
              @"completionHandler: %@",
              camera, duration, function, NSStringFromUIEdgeInsets(edgePadding), completion);
...
- (void)flyToCamera:(MLNMapCamera *)camera
         withDuration:(NSTimeInterval)duration
    completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Setting flyToCamera: %@ withDuration: %f completionHandler: %@", camera, duration,
              completion);
...
- (void)_flyToCamera:(MLNMapCamera *)camera
  ...
  if (peakAltitude >= 0) {
    CLLocationDegrees peakLatitude =
        (self.centerCoordinate.latitude + camera.centerCoordinate.latitude) / 2;
    CLLocationDegrees peakPitch = (self.camera.pitch + camera.pitch) / 2;
    animationOptions.minZoom =
        MLNZoomLevelForAltitude(peakAltitude, peakPitch, peakLatitude, self.frame.size);
  }
...
- (MLNMapCamera *)cameraForCameraOptions:(const mbgl::CameraOptions &)cameraOptions {
  ...
  auto camera = [MLNMapCamera cameraLookingAtCenterCoordinate:centerCoordinate
                                                     altitude:altitude
                                                        pitch:pitch
                                                      heading:direction];
...
- (mbgl::CameraOptions)cameraOptionsObjectForAnimatingToCamera:(MLNMapCamera *)camera
  ...
  options.zoom = MLNZoomLevelForAltitude(camera.altitude, camera.pitch,
                                         camera.centerCoordinate.latitude, self.frame.size);
  if (camera.heading >= 0) {
...
- (CGRect)convertLatLngBounds:(mbgl::LatLngBounds)bounds toRectToView:(nullable UIView *)view {
  ...
  auto center = [self convertPoint:{CGRectGetMidX(view.bounds), CGRectGetMidY(view.bounds)}
  ...
  CGRect rect = {[self convertLatLng:correctedLatLngBounds.southwest() toPointToView:view],
                 CGSizeZero};
  rect = MLNExtendRect(rect, [self convertLatLng:correctedLatLngBounds.northeast()
                                   toPointToView:view]);
  return rect;
}
...
- (nullable NSArray<id<MLNAnnotation>> *)annotations {
  ...
  std::transform(_annotationContextsByAnnotationTag.begin(),
                 _annotationContextsByAnnotationTag.end(), std::back_inserter(annotations),
...
- (nullable NSArray<id<MLNAnnotation>> *)visibleAnnotationsInRect:(CGRect)rect {
  ...
  if (shapeAnnotationTags.size()) {
    annotationTags.insert(annotationTags.end(), shapeAnnotationTags.begin(),
                          shapeAnnotationTags.end());
  }
...
- (id<MLNAnnotation>)annotationWithTag:(MLNAnnotationTag)tag {
  if (!_annotationContextsByAnnotationTag.count(tag) || tag == MLNAnnotationTagNotFound) {
    return nil;
  }
...
- (void)addAnnotations:(NSArray<id<MLNAnnotation>> *)annotations {
  ...
  BOOL delegateImplementsViewForAnnotation =
      [self.delegate respondsToSelector:@selector(mapView:viewForAnnotation:)];
  BOOL delegateImplementsImageForPoint =
      [self.delegate respondsToSelector:@selector(mapView:imageForAnnotation:)];
  ...
  for (id<MLNAnnotation> annotation in annotations) {
    ...
    if ([annotation isKindOfClass:[MLNMultiPoint class]]) {
      ...
      MLNAnnotationTag annotationTag =
          self.mbglMap.addAnnotation([multiPoint annotationObjectWithDelegate:self]);
      MLNAnnotationContext context;
      context.annotation = annotation;
      _annotationContextsByAnnotationTag[annotationTag] = context;
      _annotationTagsByAnnotation[annotation] = annotationTag;

      [(NSObject *)annotation addObserver:self
                               forKeyPath:@"coordinates"
                                  options:0
                                  context:(void *)(NSUInteger)annotationTag];
      ...
      if (delegateImplementsViewForAnnotation) {
        ...
        if (annotationView) {
          ...
          annotationView.center = MLNPointRounded([self convertCoordinate:annotation.coordinate
                                                            toPointToView:self]);
      ...
      if (!annotationView) {
        ...
        if (!annotationImage) {
          annotationImage =
              [self dequeueReusableAnnotationImageWithIdentifier:MLNDefaultStyleMarkerSymbolName];
        }
        if (!annotationImage) {
        ...
        if (!symbolName) {
          symbolName =
              [MLNAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
      ...
      if ([annotation isKindOfClass:[NSObject class]]) {
        MLNAssert(![annotation isKindOfClass:[MLNMultiPoint class]],
                  @"Point annotation should not be MLNMultiPoint.");
        [(NSObject *)annotation addObserver:self
...
- (void)updateAnnotationContainerViewWithAnnotationViews:
...
- (MLNAnnotationImage *)defaultAnnotationImage {
  ...
  MLNAnnotationImage *annotationImage =
      [MLNAnnotationImage annotationImageWithImage:image
                                   reuseIdentifier:MLNDefaultStyleMarkerSymbolName];
  annotationImage.styleIconIdentifier =
      [MLNAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
...
- (MLNAnnotationImage *)invisibleAnnotationImage {
  MLNAnnotationImage *annotationImage =
      [self dequeueReusableAnnotationImageWithIdentifier:MLNInvisibleStyleMarkerSymbolName];
...
- (MLNAnnotationView *)annotationViewForAnnotation:(id<MLNAnnotation>)annotation {
  ...
  if (annotationView) {
    ...
    CGRect bounds = UIEdgeInsetsInsetRect({CGPointZero, annotationView.frame.size},
                                          annotationView.alignmentRectInsets);
    ...
    CGFloat adjustedAnnotationHeight =
        CGRectGetHeight(bounds) + fabs(annotationView.centerOffset.dx);

    _largestAnnotationViewSize =
        CGSizeMake(MAX(_largestAnnotationViewSize.width, adjustedAnnotationWidth),
                   MAX(_largestAnnotationViewSize.height, adjustedAnnotationHeight));

    _unionedAnnotationRepresentationSize = CGSizeMake(
        MAX(_unionedAnnotationRepresentationSize.width, _largestAnnotationViewSize.width),
...
- (void)installAnnotationImage:(MLNAnnotationImage *)annotationImage {
  ...
  // add sprite
  self.mbglMap.addAnnotationImage(
      [annotationImage.image mgl_styleImageWithIdentifier:iconIdentifier]);
  ...
  _unionedAnnotationRepresentationSize =
      CGSizeMake(MAX(_unionedAnnotationRepresentationSize.width, bounds.size.width),
...
- (void)removeAnnotations:(NSArray<id<MLNAnnotation>> *)annotations {
  ...
  for (id<MLNAnnotation> annotation in annotations) {
    ...
    if ([annotation isKindOfClass:[NSObject class]] &&
      ...
      [(NSObject *)annotation removeObserver:self
                                  forKeyPath:@"coordinate"
                                     context:(void *)(NSUInteger)annotationTag];
...
- (nonnull NSArray<id<MLNOverlay>> *)overlays {
  ...
  [self.annotations enumerateObjectsUsingBlock:^(id<MLNAnnotation> _Nonnull annotation,
...
- (void)addOverlays:(NSArray<id<MLNOverlay>> *)overlays {
  ...
  for (id<MLNOverlay> overlay in overlays) {
    MLNAssert([overlay conformsToProtocol:@protocol(MLNOverlay)],
              @"overlay should conform to MLNOverlay");
  }
#endif
...
- (nullable MLNAnnotationView *)dequeueReusableAnnotationViewWithIdentifier:(NSString *)identifier {
  NSMutableArray *annotationViewReuseQueue =
      [self annotationViewReuseQueueForIdentifier:identifier];
...
- (MLNAnnotationTag)annotationTagAtPoint:(CGPoint)point persistingResults:(BOOL)persist {
  ...
  CGRect queryRect = CGRectInset({point, CGSizeZero}, -_unionedAnnotationRepresentationSize.width,
  ...
  if (nearbyShapeAnnotations.size()) {
    nearbyAnnotations.insert(nearbyAnnotations.end(), nearbyShapeAnnotations.begin(),
                             nearbyShapeAnnotations.end());
  }

  if (nearbyAnnotations.size()) {
    // Assume that the user is fat-fingering an annotation.
    CGRect hitRect = CGRectInset({point, CGSizeZero}, -MLNAnnotationImagePaddingForHitTest,
                                 -MLNAnnotationImagePaddingForHitTest);

    // Filter out any annotation whose image or view is unselectable or for which
    // hit testing fails.
    auto end = std::remove_if(
        nearbyAnnotations.begin(), nearbyAnnotations.end(),
        [&](const MLNAnnotationTag annotationTag) {
          ...
          if (annotationView) {
            ...
            CGPoint calloutAnchorPoint =
                MLNPointRounded([self convertCoordinate:annotation.coordinate toPointToView:self]);
            CGRect frame = CGRectInset({calloutAnchorPoint, CGSizeZero},
                                       -CGRectGetWidth(annotationView.frame) / 2,
                                       -CGRectGetHeight(annotationView.frame) / 2);

            // We need to take any offset into consideration. Note that a large offset will result
            // in a large value for `_unionedAnnotationRepresentationSize` (and thus a larger
            // feature query rect). Aim to keep the offset as small as possible.
            frame =
                CGRectOffset(frame, annotationView.centerOffset.dx, annotationView.centerOffset.dy);
            ...
            if ([annotation isKindOfClass:[MLNMultiPoint class]]) {
              if ([self.delegate respondsToSelector:@selector(mapView:shapeAnnotationIsEnabled:)]) {
                return !!(![self.delegate mapView:self
                         shapeAnnotationIsEnabled:(MLNMultiPoint *)annotation]);
              } else {
                return false;
              }
            }
            ...
            MLNAnnotationImage *fallbackAnnotationImage =
                [self dequeueReusableAnnotationImageWithIdentifier:MLNDefaultStyleMarkerSymbolName];
            UIImage *fallbackImage = fallbackAnnotationImage.image;

            annotationRect = [self frameOfImage:annotationImage.image ?: fallbackImage
                           centeredAtCoordinate:annotation.coordinate];
          }
  ...
  if (nearbyAnnotations.size()) {
    ...
    if (nearbyAnnotations == _annotationsNearbyLastTap) {
      ...
      if (_selectedAnnotationTag == MLNAnnotationTagNotFound ||
        ...
        auto result =
            std::find(nearbyAnnotations.begin(), nearbyAnnotations.end(), _selectedAnnotationTag);
...
- (BOOL)isMovingAnnotationIntoViewSupportedForAnnotation:(id<MLNAnnotation>)annotation
                                                animated:(BOOL)animated {
  // Consider delegating
...
- (id<MLNAnnotation>)selectedAnnotation {
  ...
  MLNAnnotationContext &annotationContext =
      _annotationContextsByAnnotationTag.at(_selectedAnnotationTag);
  return annotationContext.annotation;
}
...
- (void)setSelectedAnnotations:(NSArray<id<MLNAnnotation>> *)selectedAnnotations {
  ...
  MLNAssert([firstAnnotation conformsToProtocol:@protocol(MLNAnnotation)],
            @"annotation should conform to MLNAnnotation");
...
- (void)selectAnnotation:(id<MLNAnnotation>)annotation
       ...
       completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Selecting annotation: %@ moveIntoView: %@ animateSelection: %@", annotation,
              MLNStringFromBOOL(moveIntoView), MLNStringFromBOOL(animateSelection));
  CGRect positioningRect = [self positioningRectForAnnotation:annotation
                                          defaultCalloutPoint:CGPointZero];
  [self selectAnnotation:annotation
                moveIntoView:moveIntoView
            animateSelection:animateSelection
      calloutPositioningRect:positioningRect
           completionHandler:completion];
}
...
- (void)selectAnnotation:(id<MLNAnnotation>)annotation
              moveIntoView:(BOOL)moveIntoView
          animateSelection:(BOOL)animateSelection
    calloutPositioningRect:(CGRect)calloutPositioningRect
         completionHandler:(nullable void (^)(void))completion {
  if (!annotation) return;
  ...
  // Determine if we're allowed to move this offscreen annotation on screen, even though we've asked
  // it to
  if (moveIntoView) {
    moveIntoView = [self isMovingAnnotationIntoViewSupportedForAnnotation:annotation
  ...
  if ([annotation respondsToSelector:@selector(title)] && annotation.title &&
      ...
      if (providedCalloutView) {
        if (![providedCalloutView isKindOfClass:[UIView class]]) {
          [NSException raise:NSInvalidArgumentException
                      format:@"Callout view must be a kind of UIView"];
        }
        MLNAssert([providedCalloutView conformsToProtocol:@protocol(MLNCalloutView)],
                  @"callout view must conform to MLNCalloutView");
    ...
    if (_userLocationAnnotationIsSelected) {
      ...
      CGRect implicitAnnotationFrame =
          [self.userLocationAnnotationView.layer.presentationLayer frame];
      CGRect explicitAnnotationFrame = self.userLocationAnnotationView.frame;
      _initialImplicitCalloutViewOffset = CGPointMake(
          CGRectGetMinX(explicitAnnotationFrame) - CGRectGetMinX(implicitAnnotationFrame),
    ...
    if ([self.delegate respondsToSelector:@selector(mapView:
      ...
      if ([calloutView.leftAccessoryView isKindOfClass:[UIControl class]]) {
        UITapGestureRecognizer *calloutAccessoryTap = [[UITapGestureRecognizer alloc]
            initWithTarget:self
    ...
    // If the callout view provides inset (outset) information, we can use it to expand our
    // positioning rect, which we then use to help move the annotation on-screen if want need to.
    if (moveIntoView &&
        [calloutView respondsToSelector:@selector(marginInsetsHintForPresentationFromRect:)]) {
      UIEdgeInsets margins =
          [calloutView marginInsetsHintForPresentationFromRect:calloutPositioningRect];
  ...
  if (!expandedPositioningRectToMoveCalloutIntoViewWithMargins) {
    ...
    // protocol method.
    bounds = CGRectInset(bounds, -calloutPositioningRect.size.width,
                         -calloutPositioningRect.size.height);
  }
  ...
  if ([annotation isKindOfClass:[MLNPointAnnotation class]]) {
    ...
    CGPoint anchorPoint =
        CGPointMake(CGRectGetMidX(calloutPositioningRect), CGRectGetMidY(calloutPositioningRect));
    self.anchorCoordinateForSelectedAnnotation = [self convertPoint:anchorPoint
...
- (CGRect)positioningRectForAnnotation:(id<MLNAnnotation>)annotation
                   defaultCalloutPoint:(CGPoint)calloutPoint {
  MLNAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
  ...
  // For annotations which `coordinate` falls offscreen it will use the current tap point as anchor
  // instead.
  if (!CGRectIntersectsRect(positioningRect, self.bounds) && annotation != self.userLocation) {
    if (!CGPointEqualToPoint(calloutPoint, CGPointZero)) {
      positioningRect = CGRectMake(calloutPoint.x, calloutPoint.y, positioningRect.size.width,
                                   positioningRect.size.height);
    }
  }
...
- (CGRect)positioningRectForCalloutForAnnotationWithTag:(MLNAnnotationTag)annotationTag {
  ...
  if ([annotation isKindOfClass:[MLNMultiPoint class]]) {
    ...
    CGPoint originPoint = [self convertCoordinate:origin toPointToView:self];
    return CGRectMake(originPoint.x, originPoint.y, MLNAnnotationImagePaddingForHitTest,
                      MLNAnnotationImagePaddingForHitTest);
  }
  ...
  if (!image) {
    image =
        [self dequeueReusableAnnotationImageWithIdentifier:MLNDefaultStyleMarkerSymbolName].image;
  }
  if (!image) {
    return CGRectZero;
  }
...
- (CGRect)frameOfImage:(UIImage *)image centeredAtCoordinate:(CLLocationCoordinate2D)coordinate {
  CGPoint calloutAnchorPoint = MLNPointRounded([self convertCoordinate:coordinate
                                                         toPointToView:self]);
  CGRect frame =
      CGRectInset({calloutAnchorPoint, CGSizeZero}, -image.size.width / 2, -image.size.height / 2);
...
- (MLNAnnotationImage *)imageOfAnnotationWithTag:(MLNAnnotationTag)annotationTag {
  ...
  NSString *customSymbol =
      _annotationContextsByAnnotationTag.at(annotationTag).imageReuseIdentifier;
...
- (void)calloutViewWillAppear:(UIView<MLNCalloutView> *)calloutView {
  ...
  [UIView animateWithDuration:_userLocationAnimationCompletionDate.timeIntervalSinceNow
                   ...
                   animations:^{
                     ...
                     calloutView.frame = CGRectOffset(
                         calloutView.frame, strongSelf->_initialImplicitCalloutViewOffset.x,
...
- (void)showAnnotations:(NSArray<id<MLNAnnotation>> *)annotations animated:(BOOL)animated {
  ...
  CGFloat yPadding = (self.frame.size.height / 5 <= maximumPadding) ? (self.frame.size.height / 5)
                                                                    : maximumPadding;
  CGFloat xPadding =
      (self.frame.size.width / 5 <= maximumPadding) ? (self.frame.size.width / 5) : maximumPadding;
...
- (void)showAnnotations:(NSArray<id<MLNAnnotation>> *)annotations
            edgePadding:(UIEdgeInsets)insets
               animated:(BOOL)animated
      completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Showing: %lu annotations edgePadding: %@ animated: %@", annotations.count,
              NSStringFromUIEdgeInsets(insets), MLNStringFromBOOL(animated));
  ...
  for (id<MLNAnnotation> annotation in annotations) {
    if ([annotation conformsToProtocol:@protocol(MLNOverlay)]) {
      bounds.extend(
          MLNLatLngBoundsFromCoordinateBounds(((id<MLNOverlay>)annotation).overlayBounds));
    } else {
...
- (void)annotationImageNeedsRedisplay:(MLNAnnotationImage *)annotationImage {
  ...
  NSString *fallbackIconIdentifier =
      [MLNAnnotationSpritePrefix stringByAppendingString:fallbackReuseIdentifier];

  if (annotationImage.image) {
    // Add the new icon to the style.
    NSString *updatedIconIdentifier =
        [MLNAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
    ...
    if ([iconIdentifier isEqualToString:fallbackIconIdentifier]) {
      ...
      [self applyIconIdentifier:updatedIconIdentifier
          toAnnotationsWithImageReuseIdentifier:reuseIdentifier];
    }
  } else {
    ...
    // Update any annotations associated with the annotation image.
    [self applyIconIdentifier:fallbackIconIdentifier
        toAnnotationsWithImageReuseIdentifier:reuseIdentifier];
  }
}

- (void)applyIconIdentifier:(NSString *)iconIdentifier
...
- (void)validateLocationServices {
  ...
  if (shouldEnableLocationServices) {
    if (self.shouldRequestAuthorizationToUseLocationServices &&
...
- (NSString *)accuracyDescriptionString {
  NSDictionary *dictionary = [[NSBundle mainBundle]
  ...
  if (showsUserLocation) {
    ...
    if ([self.delegate respondsToSelector:@selector(mapView:viewForAnnotation:)]) {
      userLocationAnnotationView =
          (MLNUserLocationAnnotationView *)[self.delegate mapView:self
                                                viewForAnnotation:self.userLocation];
      if (userLocationAnnotationView &&
          ![userLocationAnnotationView isKindOfClass:MLNUserLocationAnnotationView.class]) {
        [NSException raise:MLNUserLocationAnnotationTypeException
                    format:@"User location annotation view must be a kind of "
                           @"MLNUserLocationAnnotationView. %@",
                           userLocationAnnotationView.debugDescription];
      }
    }

    self.userLocationAnnotationView =
        userLocationAnnotationView ?: [[MLNFaux3DUserLocationAnnotationView alloc] init];
    self.userLocationAnnotationView.mapView = self;
    self.userLocationAnnotationView.userLocation = self.userLocation;

    self.userLocationAnnotationView.autoresizingMask =
...
- (BOOL)isUserLocationVisible {
  if (self.userLocationAnnotationView) {
    CGPoint locationPoint = [self convertCoordinate:self.userLocation.coordinate
                                      toPointToView:self];

    CGRect locationRect =
        CGRectMake(locationPoint.x - self.userLocation.location.horizontalAccuracy,
...
- (void)setUserTrackingMode:(MLNUserTrackingMode)mode
  ...
  switch (_userTrackingMode) {
    ...
    case MLNUserTrackingModeFollowWithCourse: {
      self.userTrackingState =
          animated ? MLNUserTrackingStatePossible : MLNUserTrackingStateChanged;
  ...
  CLLocation *location;
  if (_userTrackingMode != MLNUserTrackingModeNone && (location = self.userLocation.location) &&
      self.userLocationAnnotationView) {
...
- (void)setTargetCoordinate:(CLLocationCoordinate2D)targetCoordinate
                   animated:(BOOL)animated
          completionHandler:(nullable void (^)(void))completion {
  MLNLogDebug(@"Setting targetCoordinate: %@ animated: %@",
              MLNStringFromCLLocationCoordinate2D(targetCoordinate), MLNStringFromBOOL(animated));
...
- (void)validateUserHeadingUpdating {
  BOOL canShowPermanentHeadingIndicator =
      self.showsUserHeadingIndicator &&
      self.userTrackingMode != MLNUserTrackingModeFollowWithCourse;

  if (canShowPermanentHeadingIndicator ||
...
- (void)locationManager:(__unused id<MLNLocationManager>)manager
...
- (void)didUpdateLocationWithUserTrackingAnimated:(BOOL)animated
                                completionHandler:(nullable void (^)(void))completion {
  [self didUpdateLocationWithUserTrackingDuration:animated ? MLNUserLocationAnimationDuration : 0
                                completionHandler:completion];
}
...
- (void)didUpdateLocationIncrementallyDuration:(NSTimeInterval)duration
                             completionHandler:(nullable void (^)(void))completion {
...
- (void)didUpdateLocationSignificantlyAnimated:(BOOL)animated
  ...
  if (self.zoomLevel < MLNMinimumZoomLevelForUserTracking) {
    camera.altitude = MLNAltitudeForZoomLevel(MLNDefaultZoomLevelForUserTracking, camera.pitch,
                                              camera.centerCoordinate.latitude, self.frame.size);
  }
...
- (UIEdgeInsets)edgePaddingForFollowing {
  ...
  CGRect boundsAroundCorrectPoint = CGRectOffset(bounds, correctPoint.x - CGRectGetMidX(bounds),
...
- (BOOL)locationManagerShouldDisplayHeadingCalibration:(id<MLNLocationManager>)manager {
  if (self.displayHeadingCalibration)
    [self performSelector:@selector(dismissHeadingCalibrationDisplay:)
               withObject:manager
               afterDelay:10.0];
...
- (void)locationManager:(__unused id<MLNLocationManager>)manager
  ...
  dispatch_async(dispatch_get_main_queue(), ^(void) {
    if (!self->_showsUserLocation || self.pan.state == UIGestureRecognizerStateBegan ||
        newHeading.headingAccuracy < 0)
      return;

    self.userLocation.heading = newHeading;

    if (self.showsUserHeadingIndicator ||
        self.userTrackingMode == MLNUserTrackingModeFollowWithHeading) {
      [self updateUserLocationAnnotationView];
    }
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesAtPoint:(CGPoint)point
                       inStyleLayersWithIdentifiers:(NSSet<NSString *> *)styleLayerIdentifiers {
  MLNLogDebug(@"Querying visibleFeaturesAtPoint: %@ inStyleLayersWithIdentifiers: %@",
              NSStringFromCGPoint(point), styleLayerIdentifiers);
  return [self visibleFeaturesAtPoint:point
         inStyleLayersWithIdentifiers:styleLayerIdentifiers
                            predicate:nil];
}
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesAtPoint:(CGPoint)point
  ...
  if (styleLayerIdentifiers) {
    ...
    [styleLayerIdentifiers
        enumerateObjectsUsingBlock:^(NSString *_Nonnull identifier, BOOL *_Nonnull stop) {
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesInRect:(CGRect)rect
                      inStyleLayersWithIdentifiers:(NSSet<NSString *> *)styleLayerIdentifiers {
  MLNLogDebug(@"Querying visibleFeaturesInRect: %@ inStyleLayersWithIdentifiers: %@",
              NSStringFromCGRect(rect), styleLayerIdentifiers);
  return [self visibleFeaturesInRect:rect
        inStyleLayersWithIdentifiers:styleLayerIdentifiers
                           predicate:nil];
}
...
- (void)animateWithDelay:(NSTimeInterval)delay animations:(void (^)(void))animations {
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
                 dispatch_get_main_queue(), animations);
}
...
- (void)unrotateIfNeededForGesture {
  ...
  if (self.direction != 0 && state != UIGestureRecognizerStateBegan &&
...
- (void)cameraWillChangeAnimated:(BOOL)animated {
  ...
  if (!_userLocationAnnotationIsSelected || self.userTrackingMode == MLNUserTrackingModeNone ||
    ...
    BOOL dismissesAutomatically =
        (calloutView && [calloutView respondsToSelector:@selector(dismissesAutomatically)] &&
...
- (void)cameraDidChangeAnimated:(BOOL)animated {
  ...
  if (![self isSuppressingChangeDelimiters]) {
    ...
    if (respondsToSelectorWithReason) {
      [self.delegate mapView:self
          regionDidChangeWithReason:self.cameraChangeReasonBitmask
                           animated:animated];
...
- (void)updateAnnotationViews {
  ...
  for (id<MLNAnnotation> annotation in visibleAnnotations) {
    ...
    MLNAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
    MLNAssert(annotationTag != MLNAnnotationTagNotFound,
              @"-visibleAnnotationsInRect: returned unrecognized annotation");
    MLNAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(annotationTag);
    ...
    if (!annotationView) {
      ...
      if (annotationView) {
        ...
        // New annotation (created because there is nothing to dequeue) may not have been added to
        // the container view yet. Add them here.
    ...
    if (annotationView) {
      annotationView.center =
          MLNPointRounded([self convertCoordinate:annotationContext.annotation.coordinate
                                    toPointToView:self]);
    }
  }

  MLNCoordinateBounds coordinateBounds = [self convertRect:viewPort
                                toCoordinateBoundsFromView:self];
  ...
  for (id<MLNAnnotation> annotation in offscreenAnnotations) {
    ...
    if (annotationView) {
      ...
      // Every so often (1 out of 1000 frames?) the mbgl query mechanism fails. This logic spot
      // checks the offscreenAnnotations values -- if they are actually still on screen then the
...
- (void)updateCalloutView {
  ...
  if (isAnchoredToAnnotation) {
    ...
    MLNAssert(!CGRectIsNull(positioningRect),
              @"Positioning rect should not be CGRectNull by this point");

    CGPoint centerPoint =
        CGPointMake(CGRectGetMidX(positioningRect), CGRectGetMinY(positioningRect));
...
- (void)updateAttributionAlertView {
  if (self.attributionController.presentingViewController) {
    self.attributionController.popoverPresentationController.sourceRect =
        self.attributionButton.frame;
    switch (self.attributionButtonPosition) {
      case MLNOrnamentPositionTopLeft:
      case MLNOrnamentPositionTopRight:
        [self.attributionController.popoverPresentationController
            setPermittedArrowDirections:UIMenuControllerArrowUp];
        break;
      case MLNOrnamentPositionBottomLeft:
      case MLNOrnamentPositionBottomRight:
        [self.attributionController.popoverPresentationController
            setPermittedArrowDirections:UIMenuControllerArrowDown];
        break;
    }
...
- (void)updateUserLocationAnnotationViewAnimatedWithDuration:(NSTimeInterval)duration {
  ...
  if (self.userTrackingMode != MLNUserTrackingModeNone &&
    ...
    userPoint = MLNPointRounded([self convertCoordinate:self.userLocation.coordinate
                                          toPointToView:self]);
  }
  ...
  if (CGRectContainsPoint(CGRectInset(self.bounds, -MLNAnnotationUpdateViewportOutset.width,
    ...
    dispatch_block_t animation = ^{
      if (self.selectedAnnotation == self.userLocation) {
        UIView<MLNCalloutView> *calloutView = self.calloutViewForSelectedAnnotation;
        calloutView.frame = CGRectOffset(calloutView.frame, userPoint.x - annotationView.center.x,
...
- (CGPoint)userLocationAnnotationViewCenter {
  if ([self.delegate respondsToSelector:@selector(mapViewUserLocationAnchorPoint:)]) {
    ...
    return CGPointMake(anchorPoint.x + self.contentInset.left,
                       anchorPoint.y + self.contentInset.top);
  }

  CGRect contentFrame =
      UIEdgeInsetsInsetRect(self.contentFrame, self.edgePaddingForFollowingWithCourse);
...
- (void)updateScaleBar {
  ...
  if (!self.scaleBar.hidden) {
    [(MLNScaleBar *)self.scaleBar
        setMetersPerPoint:[self metersPerPointAtLatitude:self.centerCoordinate.latitude]];
  }
}
...
- (void)prepareForInterfaceBuilder {
  ...
  self.layer.borderColor =
      [UIColor colorWithRed:59 / 255. green:178 / 255. blue:208 / 255. alpha:0.8].CGColor;
  ...
  explanationLabel.text = [NSString
      stringWithFormat:
          NSLocalizedStringWithDefaultValue(
              @"DESIGNABLE", nil, nil,
              @"To display a map which requires authoriaztion here, set %@ to your API key in %@",
              @"Instructions in Interface Builder designable; {key}, {plist file name}"),
          @"MLNApiKey", @"Info.plist"];
  ...
  [linkButton setTitle:NSLocalizedStringWithDefaultValue(
                           @"FIRST_STEPS_URL", nil, nil,
                           @"docs.mapbox.com/help/tutorials/first-steps-ios-sdk",
  ...
  [self addConstraint:[NSLayoutConstraint constraintWithItem:diagnosticView
  ...
  [self addConstraint:[NSLayoutConstraint constraintWithItem:self
                                                  ...
                                                  multiplier:1
                                                    constant:8]];
  [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-[container(20@20)]-|"
                                                               options:NSLayoutFormatAlignAllCenterY
                                                               metrics:nil
                                                                 views:views]];
  [self addConstraints:[NSLayoutConstraint
                           constraintsWithVisualFormat:@"V:|[headline]-[explanation]-[link]|"
                                               options:0
                                               metrics:nil
                                                 views:views]];
  [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[headline]|"
                                                               options:0
                                                               metrics:nil
                                                                 views:views]];
  [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[explanation]|"
                                                               options:0
                                                               metrics:nil
                                                                 views:views]];
  [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[link]|"
                                                               options:0
                                                               metrics:nil
                                                                 views:views]];
}

- (NSMutableArray<MLNAnnotationView *> *)annotationViewReuseQueueForIdentifier:
    (NSString *)identifier {
